...
namespace AmeCapture.App.Views
{
    public partial class EditorPage : ContentPage, IQueryAttributable
    {
        ...
        {
            if (query.TryGetValue("itemId", out object? itemIdObj) && itemIdObj is string itemId)
            {
        ...
        private void OnPaintSurface(object? sender, SKPaintSurfaceEventArgs e)
        {
            ...
            float scaleX = (float)e.Info.Width / _sourceBitmap.Width;
            float scaleY = (float)e.Info.Height / _sourceBitmap.Height;
        ...
        private static void DrawArrow(SKCanvas canvas, ArrowAnnotation arrow)
        {
            ...
            double headLength = arrow.StrokeWidth * 4.0;
        ...
        private void OnTouch(object? sender, SKTouchEventArgs e)
        {
            ...
            float scaleX = (float)CanvasView.CanvasSize.Width / _sourceBitmap.Width;
            float scaleY = (float)CanvasView.CanvasSize.Height / _sourceBitmap.Height;
            float scale = Math.Min(scaleX, scaleY);
        ...
        {
            if (_clipboardService == null || string.IsNullOrEmpty(_viewModel.ImagePath))
            {
                return;
            }
